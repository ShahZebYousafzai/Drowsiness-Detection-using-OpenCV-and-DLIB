...
# loop over frames fom video stream
while True:
    # grab the frames fom the threaded video file, resize it and
    # convert it to grayscale
    frame = vs.read()
    frame = imutils.resize(frame, width=500)
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

    # detect the faces in the grayscale form
    rects = detector(gray, 0)

    # loop over the face detections
    for rect in rects:
        # determine the facial landmarks for the face region,
        # then convert the facial landmarks to numpy arrays
        ...
        # compute the convex hull for the left eye and right eye, then
        ...
        # and if so, increent the blink frame counter
        if ear < EYE_AR_THRESH:
            ...
            if COUNTER >= EYE_AR_CONSEC_FRAMES:
                ...
                if not ALARM_ON:
                    ...
                    # and if so, thread to have an alarm sound played
                    # in the background
                    if args["alarm"] != "":
